-- INSERT `rocky-rawdata.anacapri_ga_attr.session_last_click_attr_table` 
...
WITH base_events AS (
  SELECT 
    ...
    treated_manual_source AS manual_source,
    treated_manual_medium AS manual_medium,
  ...
  WHERE event_date BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 31 DAY) AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
  -- WHERE event_date BETWEEN DATE_SUB('2025-12-01', INTERVAL 30 DAY) AND '2026-01-07'
  AND platform = 'WEB'
),
...
pre_atrib AS (
  SELECT 
    ...
    IF(LOWER(manual_traffic_source) = 'data not available / data not available', NULL, manual_traffic_source) AS manual_traffic_source,
    ga_session_attr,
    utm_traffic_source
  FROM (
    SELECT
...
join_purchase_data AS (
  ...
  -- WHERE event_date BETWEEN '2025-12-01' AND '2026-01-07'
...
general_info AS (
  SELECT 
    ...
    user_pseudo_id,
    COUNT(DISTINCT IF(ec_transaction_id LIKE ANY ("%nao_aplicavel%", "%(not set)%"), NULL, ec_transaction_id)) AS purchases,
